...
//    	KeyListener listener = new Gamplay();
	...
	public void paint(Graphics g)  // use to draw graphics
    {
    	...
    	g.drawString("(To Restart Press SPACE_BAR key) ", 250, 28);
    	...
    	if(totalBricks <=0) {
        	...
        	g.drawString("YOU WIN !! CONGRATULATIONS" , 230, 250);
    		...
    		g.setColor(Color.white);
    		g.setFont(new Font("serif", Font.BOLD, 30));
        	g.drawString("High Scores :  "+ highscore , 190, 350);
    	...
    	if(ballposY >570) {
        	...
        	g.setColor(Color.yellow);
        	g.setFont(new Font("serif", Font.BOLD, 20));
        	g.drawString("To give it another try press " , 230, 400);
        	g.setColor(Color.red);
        	g.setFont(new Font("serif", Font.BOLD, 20));
        	g.drawString("ENTER" , 470, 400);
        	
        	g.setColor(Color.yellow);
        	g.setFont(new Font("serif", Font.BOLD, 20));
        	g.drawString("To increase speed press " , 230, 450);
        	g.setColor(Color.red);
        	g.setFont(new Font("serif", Font.BOLD, 20));
        	g.drawString("SHIFT" , 440, 450);
    	}
	...
	@Override
	public void keyTyped(KeyEvent e) { }       // Don't need  but if removed give error becoz methods are abstract in interfaces
	@Override
	...
	public void keyPressed(KeyEvent e) {
		...
		if(e.getKeyCode()== KeyEvent.VK_SHIFT)  // to restart with level up speed
		{
			if(!play) {
				play= true;
				ballposX =120;
	...
	if(e.getKeyCode()== KeyEvent.VK_SPACE) // to restart in between
	{
		if(play) {
			play= true;
			ballposX =120;
	...
	if (e.getKeyCode()== KeyEvent.VK_ENTER)	{          // to restart the game when ended
		if(!play) {
			play= true;
			ballposX =120;
			ballposY =350;
